...
Covers forward DNS, reverse DNS, TCP socket connects, UDP datagram sendto/sendmsg,
and standard library HTTP clients (urllib.request.urlopen).
...
def _guarded_gethostbyaddr(ip_address: Any) -> Any:
    if _is_allowed_host(ip_address):
        return _REAL_GETHOSTBYADDR(ip_address)

    sanitized = _sanitize_destination(ip_address)
    raise NetworkAccessDeniedError(
        f"Outbound network access is disabled during unit tests: reverse DNS lookup for {sanitized!r}"
    )


def _guarded_getnameinfo(sockaddr: Any, flags: int) -> Any:
    if _is_allowed_socket_address(sockaddr):
        return _REAL_GETNAMEINFO(sockaddr, flags)

    sanitized = _sanitize_destination(sockaddr)
    raise NetworkAccessDeniedError(
        f"Outbound network access is disabled during unit tests: reverse DNS lookup for {sanitized!r}"
    )
...
def _guarded_socket_sendto(self: socket.socket, *args: Any, **kwargs: Any) -> int:
    address = None
...
def hermetic_network_guard(monkeypatch: pytest.MonkeyPatch, request: pytest.FixtureRequest) -> None:
    ...
    Invariant:
    - Normal tests: outbound network is blocked by default (DNS, reverse-DNS, TCP, UDP datagrams, urllib).
    ...
    monkeypatch.setattr(socket.socket, "sendto", _guarded_socket_sendto)
    if _REAL_SOCKET_SENDMSG is not None:
        monkeypatch.setattr(socket.socket, "sendmsg", _guarded_socket_sendmsg)
